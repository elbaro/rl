...
def train(config):
    wandb.init(project="lunar-lander", name="basic")
    ...
    wandb.watch([q, policy], log="all", log_freq=10000)
    ...
    while True:
        ...
        while (not is_done) and step < STEP:
            ...
            current_q_values = q(batch.states, batch.actions)
            ...
            backup = (
                GAMMA
                * (1 - batch.is_dones)
                * (q(batch.next_states, next_action) - ENTROPY_TERM_COEFFICIENT * logp)
            )
            ...
            q_loss = F.mse_loss(current_q_values, batch.rewards + backup)
            q_opt.zero_grad()
            q_loss.backward()
            q_opt.step()
            del current_q_values, backup

            # update Policy, ascend on Q+H
            action, logp = policy(
                batch.states, with_logprob=True
            )  # at this time, differentiable action
            policy_profit = q(batch.states, action) - ENTROPY_TERM_COEFFICIENT * logp
            ...
            if step % 10000 == 0:
                torch.save(
                    dict(q=q.state_dict(), policy=policy.state_dict()), "basic.pt"
                )
...
def test(config):
    ...
    policy.load_state_dict(torch.load("basic.pt")["policy"])
    policy.eval()
